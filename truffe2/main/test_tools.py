...
def initial_data():
    ...
    admin_user = TruffeUser.objects.create_superuser(username='admin', password='admin',
    ...
    SignableDocument(id=1, title='unsigned', description='descripption', file='sound/messagebox.mp3', active=True).save()
    SignableDocument(id=2, title='signed', description='descripption', file='sound/smallbox.mp3', active=True).save()
    ...
    Booking(id=1, unit=Unit.objects.get(pk=settings.ROOT_UNIT_PK),
            title="booking", responsible=admin_user, reason="why not?", provider_id=1,
            ...
            location=Location.objects.create(name="place", description="---"),
            start_date='2000-01-01', end_date='2099-12-31').save()
